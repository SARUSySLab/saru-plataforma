...
| 2026-09-13 | A PoC continua o repositório da família Piloto, sob o Lucas. Fusão em monorepo dentro do `saru` só quando a família Campeonato abrir, com o Lucas avisado e adicionado ao `saru`; o runbook em `docs/processo/runbook-fusao-monorepo.md` fica pronto para esse dia | Hoje só uma família tem código; mover o código do Lucas para repositório pessoal sem ele saber quebra a confiança; no monorepo a `main` protegida do `saru` passa a cobrir todo o código sem plano pago | Fundir agora, como o handoff do Antigravity propunha | aprovada |
| 2026-09-13 | Código de física (`parameters_gt3_cup.py` e testes) sai de `saru/docs/fisica/` e entra na PoC em `src/saru_poc/fisica/`; `docs/fisica/` do `saru` fica só com documentos | Programa e documento em pastas diferentes é o que permite testar e versionar o programa com o resto do código; o `saru` não tem código de produto | Mudar a regra do `saru` para aceitar código | aprovada |
