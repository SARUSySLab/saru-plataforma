...
| PIL-RF-13 | Declarar bloco sem dado | O sistema deve marcar cada bloco sem dado como indisponível com um motivo do vocabulário (sem_gps, sem_venue, sem_catalogo_de_curva, sem_setor, sem_contexto_pneu, sem_contexto_sessao, sem_canal_combustivel) e um texto legível | usuário | E-RF-05, E-RNF-01 | F1 fusão 2; F3 `MotivoDegradacao` | must | PIL-CT-19, PIL-CT-60 | 1 | implementado |
...
| PIL-RF-23 | Dizer onde freou e virou sem canal de pedal | O sistema deve derivar o ponto de frenagem da queda do G longitudinal e o ponto de viragem da subida do G lateral quando a gravação não tem canal de pedal, e dizer ao piloto em metros onde freou e virou em relação à referência | usuário | E-RF-05 | F4 item F1; F10 | should | PIL-CT-30 | 1 | proposto, bloqueado por PIL-RN-13 |
...
| PIL-RF-27 | Ler telemetria de simulador | O sistema deve ler a gravação exportada por simulador de corrida e entregar o mesmo relatório da volta real, na ordem iRacing, ACC, Assetto Corsa e GT7 | usuário | E-RF-01 | Vitor 2026-09-13 (F14) | must | PIL-CT-44, PIL-CT-45 | 1 | parcial: ACC e GT7 leem pelo container `.ld`; iRacing e Assetto Corsa sem leitor |
...
| PIL-RNF-10 | Confiabilidade | O instante do corte de volta tem erro menor ou igual ao período da série de maior taxa da gravação | Em 2026-09-12, 17 gravações cortam a 1 Hz com erro de até 1 s. Meta: refinar contra a série rápida; a tolerância final é decisão de Vitor (E-RN-02) | própria da família | F2 dívidas | should | PIL-CT-41, PIL-CT-58 | 1 | proposto |
...
| PIL-RN-16 | Se duas ou mais candidatas caem dentro do raio, então o degrau GPS não decide e a resolução segue para o degrau seguinte | não | F9 ADR-0048, seção "Regras que NÃO mudam". O ADR está com status proposto e aguarda ratificação desde 2026-08-20; o que Vitor decidiu naquela data foi o número do raio, não esta regra | PIL-RF-07 | 1 |
| PIL-RN-07 | Se o fator que fecha o eixo de distância no comprimento do layout ficar fora da faixa 0,9 a 1,1, então a decomposição recusa a volta com o tamanho do erro na mensagem | sim | `pipeline/decomposicao.py:141`; caso Curitiba com 67 voltas recusadas. Aprovação de Vitor pendente | PIL-RF-09 | 1 |
| PIL-RN-08 | Se uma volta cobre a pista (cobertura de distância e setores), então é válida; piso de tempo absoluto não é critério de validade | sim | F9 ADR-0033, proposto em 2026-07-27, aprovação de Vitor pendente | PIL-RF-08, PIL-RF-10 | 1 |
| PIL-RN-10 | Se um canal tem nome canônico, então tem uma única unidade de saída; peculiaridade de fonte é resolvida no alias, nunca no leitor nem no consumidor; canal sem unidade provada fica fora do canônico | sim | F9 ADR-0049; ratificação de Vitor pendente para esta família | PIL-RF-05 | 1 |
...
| PIL-RN-12 | Se a volta é in-lap ou out-lap, então fica fora do cálculo de consistência | não | F1 etapa 7 | PIL-RF-10 | 1 |
| PIL-RN-13 | Se a gravação não tem canal de pedal e tem acelerômetro válido, então o ponto de frenagem é onde o G longitudinal cai abaixo de um limiar medido no acervo; o valor do limiar só entra com aprovação de Vitor | sim | F4 item F1; F10 mediu queda mediana de 11 a 24 m/s² em 232 voltas com pedal real. Valor não aprovado | PIL-RF-23 | 1 |
...
| PIL-CT-41 | PIL-RNF-10 | uma gravação com canal de volta a 1 Hz e série de velocidade a 20 Hz | é cortada | o instante do corte tem erro menor ou igual ao período da série rápida | a escrever, após Vitor fixar a tolerância |
...
| PIL-CT-52 | 3e | PIL-RF-03, PIL-RN-11 | um `.gpk` ou `.rrk`, que o leitor lê só como inventário | é ingerido | a gravação entra sem amostra, a ingestão fica com status parcial e o relatório diz por quê | teste do leitor existe (`tests/test_reader_aim_gpk_rrk.py::test_gpk_ler_levanta_not_implemented`); o aviso no relatório é a escrever |
...
| PIL-CT-58 | 5i | PIL-RF-08, PIL-RNF-10 | uma gravação cujo único canal de volta é de 1 Hz, com série de maior taxa disponível | é cortada | o instante do corte é refinado contra a série rápida e o tempo de volta deixa de sair em segundos inteiros | a escrever; a tolerância e o critério de refino são decisão de Vitor (E-RN-02) |
...
| PIL-CT-60 | 7e | PIL-RF-13 | cada um dos sete motivos de degradação do contrato | o relatório é gerado | o bloco aparece marcado com o motivo e um texto legível, nunca some nem mostra número inventado | parcial: só `sem_catalogo_de_curva` tem teste (`tests/test_relatorio.py::test_fase_sai_declarada_como_indisponivel`); os outros seis são a escrever |
