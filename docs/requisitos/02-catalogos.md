...
| PIL-CT-52 | 3e | PIL-RF-03, PIL-RN-11 | um `.gpk` ou `.rrk`, que o leitor lê só como inventário | é ingerido | a gravação entra sem amostra, a ingestão fica com status parcial e o relatório diz por quê | `tests/test_relatorio.py::test_captura_so_de_inventario_degrada_com_motivo` e `::test_captura_com_amostra_nao_mostra_aviso_de_inventario`; no `/estado`, `tests/test_api.py::test_estado_declara_inventario` (precisa de banco) |
