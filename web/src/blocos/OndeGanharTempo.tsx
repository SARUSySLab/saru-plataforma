...
// informacoes". Um bloco, dois modos, escolhidos pelo DADO e nao pelo usuario:
// curva nomeada exige catalogo de layout, micro-setor so exige distancia.
export function OndeGanharTempo({ porCurva, porMicro, comprimento_m }: {
  ...
  const fonte = lossMode === "corner" ? porCurva : porMicro;
...
function Cabecalho({ lossMode, setLossMode, porCurva, porMicro }: {
  lossMode: "corner" | "micro";
  setLossMode: (m: "corner" | "micro") => void;
  porCurva: Talvez<{ itens: PerdaPorTrecho[] }>;
  porMicro: Talvez<{ itens: PerdaPorTrecho[] }>;
}) {
  return (
